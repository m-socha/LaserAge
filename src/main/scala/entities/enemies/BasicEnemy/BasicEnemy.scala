class BasicEnemy(startX: Int, startY: Int, randomnessPolicy: BasicEnemyRandomnessPolicy = BasicEnemyRandomnessPolicy.Default)
  ...
  def move(): Unit = {
    ...
    if (x <= 0 || x + width >= GameConfig.GAME_WIDTH) {
      direction *= -1
    } else if (randomnessPolicy.shouldRandomlyReverseXDirection()) {
      direction *= -1
    }
  }
}
